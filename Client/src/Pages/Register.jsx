...
export default function Register() {
  ...
  const [division, setDivision] = useState('Political Bureau')
    ...
    try {
      const response = await apiFetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          password,
          division,
          bio: bio || 'Accredited newsroom correspondent.',
          status
        }),
      })
      ...
      if (!response.ok) {
        throw new Error(data.message || 'This network email is already registered.')
      }

      setSuccess('Credentials registered successfully! Directing to Gateway Entrance...')
      setLoading(false)
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col lg:flex-row font-body relative overflow-hidden selection:bg-[#e30613] selection:text-white antialiased">
      ...
      <div className="hidden lg:flex lg:w-7/12 flex-col justify-between p-16 border-r border-white/5 relative z-10 bg-gradient-to-b from-transparent to-black/30">
        ...
        <div className="my-auto max-w-xl">
          ...
          >
            [ GATEWAY CORRESPONDENT REGISTRATION ]
          </motion.p>
          <motion.h1
          ...
          >
            Submit directory <span className="text-[#e30613] italic">Credentials.</span>
          </motion.h1>
          <motion.p
      ...
      <div className="w-full lg:w-5/12 flex flex-col justify-between p-8 sm:p-16 relative z-10 bg-slate-950/60 backdrop-blur-2xl overflow-y-auto">
        ...
        <div className="flex justify-between items-center w-full">
          ...
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Back to Entry Gateway
          </a>
          <div className="lg:hidden flex items-center gap-2">
            <span className="w-6 h-6 bg-[#e30613] rounded flex items-center justify-center text-white font-display font-black text-sm">B</span>
          </div>
        </div>
        ...
        <div className="my-auto py-8">
          <div className="mb-8">
            ...
            <p className="font-mono text-[10px] text-slate-400 tracking-wider uppercase font-bold flex items-center gap-1.5">
              <Shield className="w-3.5 h-3.5 text-[#e30613]" /> Newsroom Registration Console
            </p>
          </div>
          ...
          {success && (
            <motion.div 
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mb-6 bg-emerald-950/20 border border-emerald-500/20 rounded-xl p-4 flex gap-3 text-emerald-400 text-xs font-mono items-center"
            >
          ...
          <form className="space-y-4" onSubmit={handleRegister}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="relative">
                <label className="block font-mono text-[9px] uppercase tracking-widest text-slate-400 font-bold mb-1.5">
                  Full Name
                </label>
                <div className={`relative rounded-xl border transition-all duration-300 ${
                  inputFocused === 'name' ? 'border-[#e30613] bg-black/60 shadow-[0_0_15px_rgba(227,6,19,0.1)]' : 'border-white/10 bg-white/5'
                }`}>
                  ...
                  <input
                    ...
                    className="block w-full pl-10 pr-3 py-3 bg-transparent border-none rounded-xl font-mono text-xs text-white placeholder-slate-600 focus:outline-none"
              ...
              <div className="relative">
                ...
                <div className={`relative rounded-xl border transition-all duration-300 ${
                  ...
                  <input
                    ...
                    className="block w-full pl-10 pr-3 py-3 bg-transparent border-none rounded-xl font-mono text-xs text-white placeholder-slate-600 focus:outline-none"
                    placeholder="jane@bigtv.com"
                  />
                </div>
              </div>
            </div>

            {/* Row 2: Division and Vetting Status */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="relative">
                <label className="block font-mono text-[9px] uppercase tracking-widest text-slate-400 font-bold mb-1.5">
                  News Division
                ...
                <select
                  ...
                  className="block w-full px-3 py-3 border border-white/10 bg-white/5 rounded-xl font-mono text-xs text-white focus:outline-none focus:border-[#e30613] transition-colors"
                >
                  <option value="Political Bureau" className="bg-slate-900 text-white">Political Bureau</option>
                  <option value="Investigative Bureau" className="bg-slate-900 text-white">Investigative Bureau</option>
                  <option value="Regional Desk" className="bg-slate-900 text-white">Regional Desk</option>
                  <option value="National Bureau" className="bg-slate-900 text-white">National Bureau</option>
                </select>
              </div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="block w-full px-3 py-3 border border-white/10 bg-white/5 rounded-xl font-mono text-xs text-white focus:outline-none focus:border-[#e30613] transition-colors"
                >
                  <option value="PENDING VERIFICATION" className="bg-slate-900 text-white">PENDING VERIFICATION</option>
                  <option value="LIVE RECORD" className="bg-slate-900 text-white">LIVE RECORD</option>
                </select>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <textarea
                ...
                rows={2}
                maxLength={200}
                className="block w-full px-3 py-2.5 border border-white/10 bg-white/5 rounded-xl font-mono text-xs text-white placeholder-slate-600 focus:outline-none focus:border-[#e30613] transition-colors resize-none"
                placeholder="Briefly state your reporting coverage and editorial tenure..."
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="relative">
                ...
                <div className={`relative rounded-xl border transition-all duration-300 ${
                  inputFocused === 'confirmPassword' ? 'border-[#e30613] bg-black/60 shadow-[0_0_15px_rgba(227,6,19,0.1)]' : 'border-white/10 bg-white/5'
                }`}>
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500">
                    <Lock className="h-4 w-4" />
                  </div>
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full flex justify-center py-4 px-4 border border-transparent rounded-xl shadow-lg text-white font-mono text-xs tracking-widest font-bold bg-[#e30613] hover:bg-[#b0050d] transition-all duration-300 disabled:opacity-50 cursor-pointer uppercase"
              >
                {loading ? 'STORING REGISTRY...' : 'SUBMIT REGISTRATION'}
              </button>
            </div>
        ...
        <div className="border-t border-white/5 pt-6 text-center text-xs font-mono text-slate-400">
          ...
          <a href="#/login" className="text-[#e30613] hover:text-[#b0050d] transition-colors font-bold uppercase tracking-wider">
            Gateway Entrance
          </a>
        </div>
